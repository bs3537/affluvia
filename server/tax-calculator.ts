...
interface StateTaxInfo {
  ...
  seniorExemption?: number; // Additional exemption for seniors
  pensionExclusion?: number; // Some states exclude pension/retirement income
  socialSecurityTaxed?: boolean; // Whether state taxes Social Security
}
...
const STATE_TAX_INFO: { [key: string]: StateTaxInfo } = {
...
export function getTaxFriendlyStates(): string[] {
  const taxFriendlyStates: string[] = [];
...
export function compareStateTaxes(
  ...
  age: number = 65,
  spouseAge?: number
): { currentTax: number; proposedTax: number; annualSavings: number } {
