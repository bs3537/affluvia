...
// LTC States for Markov Chain Model
export type LTCState = 'healthy' | 'needs_assistance' | 'home_care' | 'assisted_living' | 'nursing_home' | 'deceased';

// LTC Insurance Policy Types
export interface LTCInsurancePolicy {
  type: 'traditional' | 'hybrid' | 'none';
  dailyBenefit: number;
  benefitPeriodYears: number; // 2, 3, 5, or lifetime (100)
  eliminationPeriodDays: number; // 30, 60, 90
  eliminationType?: 'calendar' | 'service'; // Default to calendar if not specified
  inflationProtection: 'none' | '3%_compound' | '5%_simple' | 'cpi';
  premiumAnnual: number;
  policyStartAge: number;
  sharedCareBenefit?: boolean; // For couples
  taxQualified?: boolean; // Whether policy meets HIPAA tax-qualified standards
}
...
export interface LTCEvent {
  ...
  insuranceBenefitUsed: number;
  outOfPocketCost: number;
}

// Regional Cost Adjustments (relative to national average)
export const REGIONAL_LTC_COST_FACTORS: Record<string, number> = {
...
export function getAnnualLTCProbability(
  age: number,
  gender: 'male' | 'female',
...
export function getLTCTransitionProbabilities(
  currentState: LTCState,
  age: number,
  hasInsurance: boolean
): Record<LTCState, number> {
...
export function generateLTCDuration(
  state: LTCState,
  gender: 'male' | 'female',
  hasInsurance: boolean,
  rng?: RandomSource
  ...
  const u = deriveRNG(rng, 'ltc-duration', 1).next();
  const randomFactor = -Math.log(Math.max(1e-12, u));
  return Math.max(0.25, adjustedDuration * randomFactor);
}

// Calculate annual LTC costs based on care type and location
export function calculateAnnualLTCCost(
  state: LTCState,
  retirementState: string,
  year: number,
  ltcInflationRate: number = 0.045 // Parameter kept for backward compatibility but not used
): number {
  // Get base cost for care type
  let baseCost: number;
  switch (state) {
    case 'needs_assistance':
      baseCost = BASE_LTC_COSTS.adultDayHealth * 0.5; // Part-time assistance
      break;
    case 'home_care':
      baseCost = BASE_LTC_COSTS.homeHealthAide;
      break;
    case 'assisted_living':
      baseCost = BASE_LTC_COSTS.assistedLiving;
      break;
    case 'nursing_home':
      baseCost = BASE_LTC_COSTS.nursingHomeSemi;
      break;
    default:
      baseCost = 0;
  }
  ...
  const regionalFactor = REGIONAL_LTC_COST_FACTORS[retirementState] || 1.0;
...
export function calculateLTCInsurancePremium(
  age: number,
  gender: 'male' | 'female',
  healthStatus: 'excellent' | 'good' | 'fair' | 'poor',
  policy: Omit<LTCInsurancePolicy, 'premiumAnnual'>
): number {
  // Base premium factors (per $100 daily benefit) - Updated 2024/2025
  // Calibrated against AALTCI and industry data for market accuracy
  const baseRatesPer100Daily = {
    40: 800,    // Increased from 600 (+33%) - was underpriced
    45: 1000,   // New intermediate point for better interpolation
    50: 1300,   // Increased from 900 (+44%) - was underpriced 
    55: 1200,   // Kept same - accurate per market data
    60: 1500,   // Decreased from 1800 (-17%) - was overpriced
    65: 2800,   // Kept same - accurate per market data
    70: 4500    // Kept same - reasonable for this age
  };
...
export function calculateLTCInsuranceBenefit(
  policy: LTCInsurancePolicy,
  ltcEvent: LTCEvent,
  currentYear: number
): { benefitAmount: number; remainingBenefit: number } {
  if (policy.type === 'none') {
...
export function calculateLTCTaxBenefit(
  ltcExpenses: number,
  ...
  const premiumDeductionLimits: Record<number, number> = {
  ...
  if (age <= 40) maxPremiumDeduction = premiumDeductionLimits[40];
  else if (age <= 50) maxPremiumDeduction = premiumDeductionLimits[50];
  else if (age <= 60) maxPremiumDeduction = premiumDeductionLimits[60];
  else if (age <= 70) maxPremiumDeduction = premiumDeductionLimits[70];
  else maxPremiumDeduction = premiumDeductionLimits[71];
...
export function simulateLTCTransition(
  currentState: LTCState,
  age: number,
  gender: 'male' | 'female',
  healthStatus: 'excellent' | 'good' | 'fair' | 'poor',
  ...
  if (currentState === 'healthy') {
    ...
    const rrng = deriveRNG(rng, 'ltc-transition-init', age);
    const rnd = () => rrng.next();
    if (rnd() < ltcProbability) {
      ...
      if (age < 75 && healthStatus !== 'poor') {
        ...
        if (rand < 0.5) return 'needs_assistance';
        else if (rand < 0.8) return 'home_care';
        else return 'assisted_living';
      } else {
        // Older or less healthy more likely to need higher care
        if (rand < 0.2) return 'needs_assistance';
        else if (rand < 0.5) return 'home_care';
        else if (rand < 0.8) return 'assisted_living';
        else return 'nursing_home';
      }
    }
    return 'healthy';
  }
  ...
  const rand = deriveRNG(rng, 'ltc-transition-draw', age).next();
  let cumProb = 0;
  
  for (const [nextState, prob] of Object.entries(transitions)) {
    cumProb += prob;
    if (rand <= cumProb) {
      return nextState as LTCState;
    }
  }
...
// Enhanced LTC modeling result
export interface LTCModelingResult {
  hadLTCEvent: boolean;
  totalLTCCosts: number;
  totalInsuranceBenefits: number;
  totalOutOfPocketCosts: number;
  ltcEvents: LTCEvent[];
  maxSimultaneousCost: number; // For couples
  medicaidRequired: boolean;
  yearsInLTC: number;
}

// Model LTC events throughout retirement
export function modelLTCEvents(
  startAge: number,
  endAge: number,
  gender: 'male' | 'female',
  healthStatus: 'excellent' | 'good' | 'fair' | 'poor',
  retirementState: string,
  ltcInsurance: LTCInsurancePolicy,
  spouseInfo?: {
    startAge: number;
    gender: 'male' | 'female';
    healthStatus: 'excellent' | 'good' | 'fair' | 'poor';
  ...
  const rrng2 = deriveRNG(rng, 'ltc-rc-simplified');
  const rnd = () => rrng2.next();
  const durationVariability = 0.5 + rnd() * 1.0; // 0.5 to 1.5 multiplier
  ...
  const minLTCAge = 75;
  const ltcStartAge = Math.max(
    minLTCAge + Math.floor(rnd() * (endAge - minLTCAge - LTC_DURATION_YEARS + 1)),
    startAge
  );
  ...
  const hasLTCEvent = rnd() < lifetimeLTCProbability;
  ...
  if (hasLTCEvent && ltcStartAge <= endAge) {
    ...
    for (let year = 0; year < userEvent.duration; year++) {
      ...
      if (ltcInsurance.type !== 'none') {
        const annualBenefit = Math.min(
          ltcInsurance.dailyBenefit * 365,
          yearCost
        );
  ...
  if (spouseInfo) {
    ...
    const spouseDurationVariability = 0.5 + rnd() * 1.0;
    const spouseLTCDuration = Math.min(
      spouseAvgDuration * spouseDurationVariability,
      endAge - startAge
    );
    ...
    if (spouseHasLTC && spouseLTCStartAge <= spouseEndAge) {
      ...
      for (let year = 0; year < spouseEvent.duration; year++) {
        ...
        if (spouseInfo.ltcInsurance.type !== 'none') {
          const annualBenefit = Math.min(
            spouseInfo.ltcInsurance.dailyBenefit * 365,
            yearCost
          );
...
export function calculateDeterministicLTCCosts(
  startAge: number,
  endAge: number,
  gender: 'male' | 'female',
  healthStatus: 'excellent' | 'good' | 'fair' | 'poor',
  ...
  yearlyLTCCosts: Map<number, number>;
  ...
  const yearlyLTCCosts = new Map<number, number>();
