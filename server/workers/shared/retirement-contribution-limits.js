...
export function getAgeCatchUpEligibility(birthDate) {
...
export function getContributionLimit(accountType, birthDate, includeEmployerContributions = false) {
 ...
 */
export function getFutureContributionLimit(accountType, birthDate, targetYear, includeEmployerContributions = false, annualGrowthRate = 0.02 // 2% historical average
) {
    const baseYear = 2025;
 ...
 */
export function calculateAnnualContributionsWithLimits(contributions, userBirthDate, spouseBirthDate, year) {
...
export function validateContribution(amount, accountType, birthDate, isAnnual = true) {
...
export function calculateTotalContributions(assets, monthlyContribution, employerContribution = 0) {
...
export function validateCombinedContributions(accounts, birthDate) {
    const violations = [];
    // Group accounts by contribution limit category
    const standardAccounts = accounts.filter(a => ['401k', '403b', '457b', 'governmental-457b'].includes(a.type.toLowerCase()));
    const simpleAccounts = accounts.filter(a => ['simple-401k', 'simple-ira'].includes(a.type.toLowerCase()));
    const iraAccounts = accounts.filter(a => ['traditional-ira', 'roth-ira'].includes(a.type.toLowerCase()));
