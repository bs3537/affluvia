...
function taxableSocialSecurity(ssGross, otherIncome, filingStatus) {
    if (ssGross <= 0)
        return 0;
...
function fplAmount(year, householdSize, state) {
...
function acaExpectedContributionPct(incomeAsFpl) {
    if (incomeAsFpl <= 1.5)
        return 0.00;
    if (incomeAsFpl <= 2.0)
        return 0.02 * (incomeAsFpl - 1.5) / 0.5;
    if (incomeAsFpl <= 2.5)
        return 0.02 + (0.04 - 0.02) * (incomeAsFpl - 2.0) / 0.5;
    if (incomeAsFpl <= 3.0)
        return 0.04 + (0.06 - 0.04) * (incomeAsFpl - 2.5) / 0.5;
    if (incomeAsFpl <= 4.0)
        return 0.06 + (0.085 - 0.06) * (incomeAsFpl - 3.0) / 1.0;
    return 0.085;
}
function acaRepaymentCap(fplRatio, filingStatus) {
    // Simplified 2024 repayment caps (approximate, USD). HoH treated as single.
    const isMarried = filingStatus === 'married';
    if (fplRatio <= 2.0)
        return isMarried ? 650 : 325;
    if (fplRatio <= 3.0)
        return isMarried ? 1300 : 650;
    if (fplRatio <= 4.0)
        return isMarried ? 2800 : 1400;
    return Infinity;
}
export function calculateUnifiedTaxes(inp) {
    const year = inp.year;
    ...
    if (isMarried) {
        let seniorAdd = 0;
        if (age >= 65)
            seniorAdd += cfg.seniorAdditionalDeduction.married;
        if (spouseAge >= 65)
            seniorAdd += cfg.seniorAdditionalDeduction.married;
        ...
        if (age >= 65)
            standardDeduction += cfg.seniorAdditionalDeduction.single;
    }
    ...
    const deductionType = chooseItemized ? 'itemized' : 'standard';
    ...
    if ((inp.earnedIncome || 0) > 0) {
        ...
        const base = Math.max(0, inp.earnedIncome - threshold);
