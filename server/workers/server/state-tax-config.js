...
function computeStateTaxableSocialSecurity(ssGross, otherIncome, filingStatus, state, age) {
    ...
    if (provisional <= thresholds.first)
        federalTaxable = 0;
    else if (provisional <= thresholds.second)
        federalTaxable = Math.min((provisional - thresholds.first) * 0.5, ssGross * 0.5);
    else {
    ...
    if (st === 'MT') {
        ...
        if (filingStatus === 'married') {
            if (agi < 32000)
                ...
                deductiblePct = 0.15; // only 15% deductible above 44k
        }
        else {
            if (agi < 25000)
                deductiblePct = 1.0;
...
export function calculateStateTax(grossIncome, pensionIncome, socialSecurityIncome, filingStatus, state, age) {
    ...
    const stateFiling = filingStatus === 'married' ? 'married' : 'single';
...
export function calculateCombinedTaxRate(grossIncome, pensionIncome, socialSecurityIncome, capitalGains, filingStatus, state, federalTax, age) {
    const stateResult = calculateStateTax(grossIncome, pensionIncome, socialSecurityIncome, filingStatus, state, age);
