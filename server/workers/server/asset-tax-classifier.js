...
export const ASSET_TAX_CLASSIFICATION = {
...
export function categorizeAssetsByTax(assets) {
    const buckets = {
        taxDeferred: 0,
        taxFree: 0,
        capitalGains: 0,
    ...
    for (const asset of assets) {
        ...
        if (!taxInfo || !taxInfo.isRetirementAsset || value <= 0)
            continue;
...
export function calculateOptimalRothConversion(buckets, age, currentTaxableIncome, filingStatus) {
...
export function calculateTaxEfficientWithdrawal(netNeeded, // Amount needed after taxes
buckets, taxRate, // Dynamic tax rate, not fixed 25%
age, capitalGainsRate = 0.15, // Assume 15% LTCG rate
hasQualifiedMedicalExpenses = false // For HSA treatment
) {
    const strategy = {
        fromCash: 0,
        fromCapitalGains: 0,
...
export function estimateBlendedTaxRate(buckets, ordinaryTaxRate, capitalGainsRate = 0.15) {
    if (buckets.totalAssets === 0)
        return ordinaryTaxRate;
    ...
    const blendedRate = (taxDeferredWeight * ordinaryTaxRate) +
...
export function calculateAssetWeightedReturns(assets, defaultStockReturn = 0.07, defaultBondReturn = 0.04) {
    ...
    for (const asset of assets) {
        ...
        if (value <= 0)
            continue;
        const taxInfo = ASSET_TAX_CLASSIFICATION[asset.type];
        if (!taxInfo || !taxInfo.isRetirementAsset)
            continue;
        totalValue += value;
