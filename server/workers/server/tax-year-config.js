...
function inflateTaxConfig(baseConfig, targetYear, cpiInflationRate) {
...
export function getTaxConfig(year, cpiInflationRate) {
    // Check cache first
    const cacheKey = year;
...
export function calculateFederalTaxWithYear(grossIncome, filingStatus, year) {
    if (grossIncome <= 0)
        return 0;
...
export function calculateCapitalGainsTaxWithYear(capitalGainsIncome, ordinaryIncome, filingStatus, year) {
    ...
    let previousMax = 0;
    for (const bracket of brackets) {
        if (ordinaryIncome >= bracket.max)
            continue;
        const gainsTaxableAtThisRate = Math.min(capitalGainsIncome, Math.max(0, bracket.max - Math.max(ordinaryIncome, previousMax)));
...
export function calculateIRMAAWithYear(modifiedAGI, filingStatus, year) {
    const config = getTaxConfig(year);
