...
export const REGIONAL_LTC_COST_FACTORS = {
...
export function getAnnualLTCProbability(age, gender, healthStatus) {
...
export function getLTCTransitionProbabilities(currentState, age, hasInsurance) {
...
export function generateLTCDuration(state, gender, hasInsurance, rng) {
...
export function calculateAnnualLTCCost(state, retirementState, year, ltcInflationRate = 0.045 // Parameter kept for backward compatibility but not used
) {
...
export function calculateLTCInsurancePremium(age, gender, healthStatus, policy) {
...
export function calculateLTCInsuranceBenefit(policy, ltcEvent, currentYear) {
    if (policy.type === 'none') {
...
export function calculateLTCTaxBenefit(ltcExpenses, insuranceBenefits, insurancePremiums, adjustedGrossIncome, age, policy) {
...
export function simulateLTCTransition(currentState, age, gender, healthStatus, hasInsurance, rng) {
...
export function modelLTCEvents(startAge, endAge, gender, healthStatus, retirementState, ltcInsurance, spouseInfo, rng) {
    ...
    const minLTCAge = 75;
    const ltcStartAge = Math.max(minLTCAge + Math.floor(rnd() * (endAge - minLTCAge - LTC_DURATION_YEARS + 1)), startAge);
...
export function calculateDeterministicLTCCosts(startAge, endAge, gender, healthStatus, ltcInsurance, spouseInfo) {
