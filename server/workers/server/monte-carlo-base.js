...
import { categorizeAssetsByTax, calculateTaxEfficientWithdrawal, estimateBlendedTaxRate } from './asset-tax-classifier.ts';
import { simulateSurvival, simulateCouplesSurvival } from './mortality-tables.ts';
// Glide path portfolio allocation calculation
function getGlidePathAllocation(yearsLeft, riskProfile) {
    let equityWaypoints;
    let baseReturn;
    ...
    let equity;
    if (yearsLeft >= 17)
        equity = equityWaypoints[17];
    else if (yearsLeft >= 13)
        equity = equityWaypoints[13];
    else if (yearsLeft >= 9)
        equity = equityWaypoints[9];
    else if (yearsLeft >= 5)
        equity = equityWaypoints[5];
    else if (yearsLeft >= 1)
        equity = equityWaypoints[1];
    else if (yearsLeft === 0)
        equity = equityWaypoints[0];
    else
        equity = equityWaypoints[-1];
...
function project529GrowthWithGlidePath(initialBalance, monthlyContrib, yearsToCollege, riskProfile) {
...
function runScenario(params) {
    const { currentSavings, monthlyContribution, yearsUntilStart, expectedReturn, returnVolatility = 0.15, // Default 15% volatility
    riskProfile = 'moderate' } = params;
...
export function runMonteCarloSimulation(params, iterations = 1000 // Industry standard for balance between accuracy and performance
) {
    const results = [];
...
export async function calculateEducationProjectionWithMonteCarlo(goal, profile) {
    ...
    if (goal.riskProfile === 'glide') {
        const glideData = project529GrowthWithGlidePath(currentSavings, monthlyContribution, yearsUntilStart, 'moderate');
...
export function analyzeEducationScenarios(baseParams, scenarios) {
    // Run base scenario
...
function getGlidePathTarget(yearsToRetirement) {
    if (yearsToRetirement >= 30)
        return GLIDE_PATH_SCHEDULE[30];
    if (yearsToRetirement >= 25)
        return GLIDE_PATH_SCHEDULE[25];
    if (yearsToRetirement >= 20)
        return GLIDE_PATH_SCHEDULE[20];
    if (yearsToRetirement >= 15)
        return GLIDE_PATH_SCHEDULE[15];
    if (yearsToRetirement >= 10)
        return GLIDE_PATH_SCHEDULE[10];
    if (yearsToRetirement >= 5)
        return GLIDE_PATH_SCHEDULE[5];
    if (yearsToRetirement >= 0)
        return GLIDE_PATH_SCHEDULE[0];
    if (yearsToRetirement >= -5)
        return GLIDE_PATH_SCHEDULE[-5];
...
function calculateCurrentAllocation(profileData) {
...
function calculateSpouseCurrentAllocation(profileData) {
...
function calculateReturnFromAllocation(allocation) {
    ...
    const weightedReturn = (allocation.stocks / 100) * stockReturn +
...
function getRiskProfileReturn(riskScore) {
...
function getRiskProfileAllocation(riskScore) {
    switch (riskScore) {
        case 1: // Conservative
...
function getGlidePathReturn(yearsToRetirement) {
...
function calculateAllocationReturn(allocation, inflationRate = 0.03) {
...
function calculateOwnerSpecificReturns(userAssets, spouseAssets, jointAssets, userAllocation, spouseAllocation, inflationRate = 0.03) {
...
function getAssetAllocationReturn(stockAllocation, bondAllocation, cashAllocation, expectedReturn, // Use the passed-in expected return (already inflation-adjusted)
returnVolatility // Use the passed-in volatility
) {
    ...
    const portfolioVolatility = Math.sqrt(Math.pow(stockAllocation * stockVolatility, 2) +
...
function getInitialRegime(yearsToRetirement) {
...
function generateRegimeReturn(currentRegime, stockAllocation, bondAllocation, yearsToRetirement) {
    ...
    if (yearsToRetirement !== undefined && yearsToRetirement <= 5 && yearsToRetirement >= 0) {
        ...
        if (currentRegime === 'normal' || currentRegime === 'bull') {
            ...
            for (const key in adjustedRegime.transitionProbs) {
                adjustedRegime.transitionProbs[key] /= total;
            }
        }
    }
...
function runRetirementScenario(params) {
    const { currentAge, retirementAge, lifeExpectancy, currentRetirementAssets, annualGuaranteedIncome: initialGuaranteedIncome, annualRetirementExpenses, annualHealthcareCosts = 0, spouseAge, spouseLifeExpectancy, expectedReturn, returnVolatility, inflationRate, stockAllocation, bondAllocation, cashAllocation, withdrawalRate, useGuardrails = false, taxRate, annualSavings, legacyGoal = 0 } = params;
    ...
    let userBuckets = params.userAssetBuckets ? {
        ...params.userAssetBuckets
    } : undefined;
    let spouseBuckets = params.spouseAssetBuckets ? {
        ...params.spouseAssetBuckets
    } : undefined;
    let jointBuckets = params.jointAssetBuckets ? {
    ...
    for (let accumYear = 0; accumYear < yearsToRetirement; accumYear++) {
        ...
        let annualReturn;
        const regimeResult = generateRegimeReturn(currentMarketRegime, currentStockAllocation, currentBondAllocation, yearsUntilRetirement);
        ...
        if ((params.useCurrentAllocation || (params.userAllocation && params.spouseAllocation)) &&
            ...
            const baseOwnerReturns = calculateOwnerSpecificReturns(userBuckets.totalAssets, spouseBuckets.totalAssets, jointBuckets.totalAssets, params.userAllocation, params.spouseAllocation, params.inflationRate);
        ...
        if (dynamicAnnualSavings > 0) {
            const allocationResult = allocateContributionsToOwnerBuckets(params, dynamicAnnualSavings, currentYearAge, accumYear, userBuckets, spouseBuckets, jointBuckets);
    ...
    let baseDiscretionaryExpenses = Math.max(baseNonHealthcareCosts * discretionaryRatio, enableDynamic ? minDiscretionary : 0);
    ...
    while ((userAlive || spouseAlive || portfolioBalance > 0) && distYear < maxSimulationYears) { // Cap at 60 years of retirement
        ...
        // Calculate annual return with market regime switching for sequence of returns risk
        let annualReturn;
        const regimeResult = generateRegimeReturn(currentMarketRegime, currentStockAllocation, currentBondAllocation, 0 // In retirement, sequence risk is ongoing
        );
        currentMarketRegime = regimeResult.nextRegime;
        const regimeReturn = regimeResult.return;
        ...
        const filingStatus = params.filingStatus || ((params.spouseAge !== undefined) ? 'married' : 'single');
        ...
        const capitalGainsTax = calculateCapitalGainsTax(estimatedCapitalGains, totalEstimatedIncome, cgFS);
        ...
        const withdrawalStrategy = calculateTaxEfficientWithdrawal(netExpensesNeeded, currentBuckets, taxRate, age, effectiveCapitalGainsRate // Use calculated rate instead of hard-coded 15%
        );
        ...
        const finalWithdrawalStrategy = (adjustedNetExpenses !== netExpensesNeeded) ?
            calculateTaxEfficientWithdrawal(adjustedNetExpenses, currentBuckets, taxRate, age, effectiveCapitalGainsRate) : withdrawalStrategy;
        ...
        if (spouseAge !== undefined) {
            ...
            const survivalResult = simulateCouplesSurvival({ currentAge: age, healthStatus }, { currentAge: currentSpouseAge || 0, healthStatus: spouseHealthStatus });
...
function allocateContributionsToOwnerBuckets(params, dynamicAnnualSavings, currentYearAge, accumYear, userBuckets, spouseBuckets, jointBuckets) {
...
export function runRetirementMonteCarloSimulation(params, iterations = 1000 // Industry standard for balance between accuracy and performance
) {
    const results = [];
...
function calculateProjectedRetirementPortfolio(params) {
...
function calculateDebtPayoffSavingsIncrease(profileData, currentAge, targetYear) {
    ...
    liabilities.forEach((debt) => {
        ...
        if (monthlyPayment > 0 && balance > 0) {
            ...
            const monthsToPayoff = Math.ceil(Math.log(1 + (balance * (interestRate / 1200)) / monthlyPayment) /
    ...
    if (primaryMortgage > 0 && profileData.primaryResidence?.mortgageBalance) {
        ...
        if (mortgageBalance > 0 && mortgageRate > 0) {
            const monthsToPayoff = Math.ceil(Math.log(1 + (mortgageBalance * (mortgageRate / 1200)) / primaryMortgage) /
...
export function profileToRetirementParams(profileData) {
    ...
    const retirementEligibleAssets = [
        ...
        'taxable-brokerage', 'Brokerage Account',
    ...
    const stockAllocation = totalAssets > 0 ? ((userAssetTotal * userStocks / 100) +
        (spouseAssetTotal * spouseStocks / 100) +
        (jointAssetTotal * ((jointAllocationWeight * userStocks + (1 - jointAllocationWeight) * spouseStocks) / 100))) / totalAssets : 0.6;
    const bondAllocation = totalAssets > 0 ? ((userAssetTotal * userBonds / 100) +
        (spouseAssetTotal * spouseBonds / 100) +
        (jointAssetTotal * ((jointAllocationWeight * userBonds + (1 - jointAllocationWeight) * spouseBonds) / 100))) / totalAssets : 0.35;
    const cashAllocation = totalAssets > 0 ? ((userAssetTotal * userCash / 100) +
        (spouseAssetTotal * spouseCash / 100) +
        (jointAssetTotal * ((jointAllocationWeight * userCash + (1 - jointAllocationWeight) * spouseCash) / 100))) / totalAssets : 0.05;
    ...
    const assetBuckets = categorizeAssetsByTax(allAssets.filter((asset) => {
        const owner = (asset.owner || 'user').toLowerCase();
        const value = Number(asset.value) || 0;
    ...
    const filingStatus = intakeFS === 'head_of_household' ? 'head_of_household' : (isMarriedOrPartnered ? 'married' : 'single');
    ...
    const baseTaxRate = calculateCombinedTaxRate(estimatedRetirementIncome > 0 ? estimatedRetirementIncome : totalAnnualIncome, retirementState, filingStatus, true, // isRetired
    ...
    const effectiveBlendedRate = taxDeferredRatio * baseTaxRate + // Tax-deferred at full rate
    ...
    const params = {
        ...
        ltcModeling: {
            ...
            gender: (profileData.userGender === 'female' || profileData.gender === 'F') ? 'F' : 'M',
            maritalStatus: profileData.maritalStatus || 'single',
            familySupport: profileData.familySupport || 'Medium',
