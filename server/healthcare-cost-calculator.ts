...
export interface HealthcareCostParams {
  currentAge: number;
  ...
  coverageEndAge?: number; // Age when employer coverage ends
}

export interface HealthcareCostResult {
  annualHealthcareCosts: number; // Annual cost at retirement age
  totalLifetimeHealthcareCosts: number; // Total projected lifetime costs
  medicarePartBPremium: number;
  medicarePartDPremium: number;
  medigapPremium: number;
  outOfPocketCosts: number;
  preMedicareCosts?: number; // If retiring before 65
  breakdown: {
    dental: number;
    vision: number;
    hearing: number;
...
const MEDICARE_COSTS_2025 = {
  ...
  partA: {
    ...
    hospitalDaily: { // Daily coinsurance
      days1to60: 0,
      days61to90: 419,
...
export function calculateHealthcareCosts(params: HealthcareCostParams): HealthcareCostResult {
  const {
    currentAge,
    retirementAge,
    lifeExpectancy,
  ...
  if (isMarried) {
    ...
    Object.keys(breakdown).forEach(key => {
      breakdown[key as keyof typeof breakdown] *= 2;
    });
  }
...
export function projectHealthcareCostsOverTime(
  params: HealthcareCostParams
): Array<{
  year: number;
  age: number;
  annualCost: number;
...
export function addHealthcareCostsToRetirementExpenses(
  baseMonthlyExpenses: number,
  healthcareCostParams: HealthcareCostParams
): {
  totalMonthlyExpenses: number;
  healthcarePortion: number;
