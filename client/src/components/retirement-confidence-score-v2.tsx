...
// Props interface not needed - using inline type definition

export function RetirementConfidenceScoreV2({ 
  isExpanded, 
  onToggle 
  ...
  const calculateMonteCarlo = async () => {
    ...
    try {
      ...
      if (result && (
        ...
        // Normalize probability fields to 0-100 for display
        let displayProbability = 0;
        if (typeof result.probability === 'number') {
          displayProbability = result.probability; // already 0-100
        } else if (typeof result.probabilityOfSuccess === 'number') {
          // Enhanced engine may return decimal 0-1 here
          displayProbability = result.probabilityOfSuccess > 1 ? result.probabilityOfSuccess : (result.probabilityOfSuccess * 100);
        } else if (typeof result.successProbability === 'number') {
          ...
          <>
            ...
            <div className="space-y-4">
              {/* Early Retirement Notification - Show from either saved Monte Carlo result or optimalRetirementAge prop */}
                {(() => {
                  // Use optimalRetirementAge from monteCarloResult
