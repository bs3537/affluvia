...
export function SocialSecurityClaimingComparison({ profile, isLocked = false }: Props) {
  ...
  const getClaimingScenarios = (): ClaimingScenario[] => {
    ...
    if (isMarried && userRetirementAge !== spouseRetirementAge) {
      // Find scenario where each spouse claims at their respective retirement age
      const retirementScenario = ageAnalysis.find(s => 
        s.userAge === userRetirementAge && s.spouseAge === spouseRetirementAge
      );
      if (retirementScenario) {
        scenarios.push({
          label: `Retirement Ages (${userRetirementAge}/${spouseRetirementAge})`,
          age: `${userRetirementAge}/${spouseRetirementAge}`,
          cumulative: retirementScenario.combinedCumulative,
          monthly: retirementScenario.combinedMonthly,
          description: 'Each spouse claims at retirement',
          color: '#F59E0B', // amber
        });
      }
    } else if (earliestRetirementAge !== 62 && earliestRetirementAge !== 67 && earliestRetirementAge !== 70) {
      // Single or same retirement age for both spouses
      const retirementScenario = findScenario(earliestRetirementAge, isMarried ? earliestRetirementAge : undefined);
      if (retirementScenario) {
        scenarios.push({
          label: `Retirement Age (${earliestRetirementAge})`,
          age: earliestRetirementAge.toString(),
          ...
          description: 'Claiming at planned retirement',
          color: '#F59E0B', // amber
        });
      }
    }
    ...
    // 4. Optimal Age
    const optimalScenario = findScenario(
      combined.optimalUserAge, 
      isMarried ? combined.optimalSpouseAge : undefined
    );
    if (optimalScenario) {
      const optimalLabel = isMarried && combined.optimalSpouseAge !== combined.optimalUserAge
        ? `Optimal (${combined.optimalUserAge}/${combined.optimalSpouseAge})`
        : `Optimal Age (${combined.optimalUserAge})`;
      
      scenarios.push({
        label: optimalLabel,
        age: isMarried && combined.optimalSpouseAge !== combined.optimalUserAge
          ? `${combined.optimalUserAge}/${combined.optimalSpouseAge}`
          : combined.optimalUserAge.toString(),
        cumulative: optimalScenario.combinedCumulative || optimalScenario.userCumulative,
        monthly: optimalScenario.combinedMonthly || optimalScenario.userMonthly,
        description: 'Maximizes lifetime benefits',
        isOptimal: true,
                ...
                {scenarios.length > 1 && (
                  <div className="mt-4 p-4 bg-gray-900/50 rounded-lg border border-gray-700">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="text-xs text-gray-400 mb-1">vs. Retirement Age</p>
                        {(() => {
                          // Find the retirement age scenario
                          const userRetirementAge = profile.desiredRetirementAge || 65;
                          const spouseRetirementAge = profile.spouseDesiredRetirementAge || 65;
                          const isMarried = profile.maritalStatus === 'married' || profile.maritalStatus === 'partnered';
                          
                          // Look for retirement scenario in the list - be more specific
                          let retirementScenario;
                          
                          if (isMarried && userRetirementAge !== spouseRetirementAge) {
                            // Look for the scenario with both retirement ages
                            retirementScenario = scenarios.find(s => 
                              s.label === `Retirement Ages (${userRetirementAge}/${spouseRetirementAge})`
                            );
                          } else {
                            // Look for single retirement age scenario
                            const retirementAge = isMarried ? Math.min(userRetirementAge, spouseRetirementAge) : userRetirementAge;
                            retirementScenario = scenarios.find(s => 
                              s.label === `Retirement Age (${retirementAge})`
                            );
                          }
                          ...
                          if (!retirementScenario) {
                            retirementScenario = scenarios.find(s => 
                              s.label.includes('Retirement Age') && !s.label.includes('Full')
                            );
                          }
                          
                          const difference = retirementScenario ? scenarios[0].cumulative - retirementScenario.cumulative : 0;
                          const percentIncrease = retirementScenario && retirementScenario.cumulative > 0 ? (difference / retirementScenario.cumulative * 100) : 0;
                    ...
                    <div className="text-xs text-blue-300">
                      <p>
                        ...
                        {profile.maritalStatus === 'married' || profile.maritalStatus === 'partnered' ? 
                          profile.desiredRetirementAge !== profile.spouseDesiredRetirementAge ?
                            ' For couples with different retirement ages, the "Retirement Ages" scenario shows benefits when each spouse claims at their individual retirement age.' :
                            '' :
                          ''
                        }
                      </p>
