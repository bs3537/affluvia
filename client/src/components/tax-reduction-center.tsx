      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="ai-roth-insights" className="space-y-6">
          ...
          {(() => {
            ...
            return (
              <>
                {/* Roth Conversion Readiness Assessment - Results First */}
                <Card 
                  key={`roth-assessment-${userGapYears}-${spouseGapYears}-${userYearsToRetirement}-${spouseYearsToRetirement}`}
                  className={`border-2 ${result.bgColor} ${result.borderColor}`}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div 
                        className="cursor-pointer hover:bg-white/5 rounded p-2 -m-2 transition-colors"
                        onClick={() => setShowReadinessAssessment(!showReadinessAssessment)}
                      >
                        <CardTitle className="text-white flex items-center">
                          <Target className="h-6 w-6 mr-2 text-blue-300" />
                          Roth Conversion Readiness Assessment
                        </CardTitle>
                        <CardDescription className="text-gray-400">
                          Comprehensive analysis of your Roth conversion opportunity and timing
                        </CardDescription>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Button
                        ...
                        <Button
                        onClick={async () => {
                          // Clear cached AI analysis result
                          setAiAnalysisResult(null);
                          
                          // Just invalidate queries to force a fresh fetch (more efficient than removing)
                          await queryClient.invalidateQueries({ queryKey: ["/api/financial-profile", user?.id] });
                          await queryClient.invalidateQueries({ queryKey: ["rothAnalysis", user?.id] });
                          await queryClient.invalidateQueries({ queryKey: ["aiRothAnalysis", user?.id] });
                          await queryClient.invalidateQueries({ queryKey: ["stored-roth-conversion-analysis", user?.id] });
                          
                          // Refetch immediately without delay
                          refetchUserProfile();
                        }}
                        variant="outline"
                        size="sm"
                  ...
                  {false && (
                    ...
                    {isLoadingProfile || isRefetching ? (
                      <div className="text-center py-16">
                        <Loader2 className="h-12 w-12 animate-spin mx-auto text-blue-300 mb-4" />
                        <h3 className="text-lg font-semibold text-white mb-2">
                          {isRefetching ? 'Refreshing Assessment...' : 'Loading Assessment...'}
                        </h3>
                        <p className="text-gray-400">Checking your Roth conversion readiness</p>
                      </div>
                    ) : (
                    <div className="space-y-6">
                      {/* Overall Assessment Result - Traffic Light Style */}
                      <div className={`p-6 rounded-xl border-2 ${result.borderColor} ${result.bgColor}`}>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center space-x-4">
                            <div className={`p-3 rounded-full ${result.bgColor}`}>
                          ...
                          <div className="text-right">
                            <div className="flex space-x-2 mb-2">
                              <div className={`w-4 h-4 rounded-full ${result.status === 'READY' ? 'bg-emerald-400' : 'bg-gray-600'}`}></div>
                              <div className={`w-4 h-4 rounded-full ${result.status === 'PARTIAL' ? 'bg-blue-500' : 'bg-gray-600'}`}></div>
                              <div className={`w-4 h-4 rounded-full ${result.status === 'NOT READY' ? 'bg-slate-400' : 'bg-gray-600'}`}></div>
                            </div>
                            <p className="text-xs text-gray-400">Assessment Status</p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                          <div className="bg-gray-800/50 rounded-lg p-3">
                            <div className="text-sm text-gray-400 mb-1">Golden Window Opportunity</div>
                            <div className={`font-semibold ${anyonePassesAnyTest ? 'text-emerald-400' : 'text-slate-400'}`}>
                              {anyonePassesAnyTest ? '✓ Available' : '✗ Not Available'}
                            </div>
                          </div>
                          <div className="bg-gray-800/50 rounded-lg p-3">
                            <div className="text-sm text-gray-400 mb-1">Planning Window</div>
                            <div className={`font-semibold ${anyonePassesAllTests ? 'text-emerald-400' : 'text-blue-300'}`}>
                              {anyonePassesAllTests ? '✓ Optimal Timing' : '⚠ Review Timing'}
                            </div>
                          </div>
                        </div>

                        {/* Scenario-Specific Information - Moved Above Action Items */}
                        {result.status === "READY" && (
                          <div className="mt-6 p-4 bg-emerald-900/10 rounded-lg border border-emerald-500/20">
                            <div className="flex items-start">
                              <CheckCircle className="h-5 w-5 text-emerald-400 mr-2 mt-0.5 flex-shrink-0" />
                              <div>
                                <h5 className="text-emerald-300 font-medium mb-1 text-sm">🎉 Congratulations - Perfect Setup!</h5>
                                <p className="text-emerald-200 text-sm">
                                  You have the ideal combination: {Math.max(userGapYears, spouseGapYears)} golden window years for conversions AND enough planning time to execute perfectly. 
                                  This setup could potentially save you tens of thousands in lifetime taxes while creating tax-free wealth for your heirs.
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        {result.status === "PARTIAL" && (
                          <div className="mt-6 p-4 bg-yellow-900/10 rounded-lg border border-yellow-500/20">
                            <div className="flex items-start">
                              <Info className="h-5 w-5 text-yellow-400 mr-2 mt-0.5 flex-shrink-0" />
                              <div>
                                <h5 className="text-yellow-300 font-medium mb-1 text-sm">💡 The Big Picture:</h5>
                                <p className="text-yellow-200 text-sm">
                                  {(userPassesGapTest || spousePassesGapTest) ? 
                                    `Roth conversions let you "pre-pay" taxes now while you're in lower tax brackets, then enjoy tax-free withdrawals forever. You have ${Math.max(userGapYears, spouseGapYears)} golden window years when your income (and tax rates) will likely be lower - that's your perfect conversion opportunity! You can start planning now at age 50+.` :
                                    "Roth conversions are like paying taxes at today's 'sale price' instead of tomorrow's full price. Consider creating a golden window by adjusting your retirement timeline - retiring earlier or delaying Social Security can unlock this powerful tax strategy."
                                  }
                                </p>
                              </div>
                            </div>
                        ...
                        {result.status === "NOT READY" && (
                          <div className="mt-6 p-4 bg-purple-900/10 rounded-lg border border-purple-500/20">
                            <div className="flex items-start">
                              <Lightbulb className="h-5 w-5 text-purple-400 mr-2 mt-0.5 flex-shrink-0" />
                              <div>
                                <h5 className="text-purple-300 font-medium mb-1 text-sm">💰 The Opportunity Cost:</h5>
                                <p className="text-purple-200 text-sm">
                                  Think of Roth conversions like switching from paying rent (taxes forever) to buying a house (pay taxes once, own it forever). 
                                  Even small timeline changes could unlock this strategy. For example: retiring at 63 instead of 65, or delaying Social Security from 65 to 67 creates a 2-4 year golden window worth potentially $50,000-$200,000+ in lifetime tax savings.
                                </p>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-600">
                          ...
                          {/* Add button to Retirement Prep Center when user needs to create golden window */}
                          {result.status === "PARTIAL" && 
                           !(userPassesGapTest || spousePassesGapTest) && 
                           (userPasses3YearTest || spousePasses3YearTest) && (
                            <div className="mt-4">
                              <Button
                                onClick={() => window.location.href = '/retirement-prep'}
                                className="w-full bg-gradient-to-r from-purple-500 to-purple-600 text-white hover:from-purple-600 hover:to-purple-700 transition-all duration-300"
                              >
                                <Target className="h-4 w-4 mr-2" />
                                Optimize Social Security Timing in Retirement Prep Center
                              </Button>
                              <p className="text-xs text-gray-400 mt-2 text-center">
                                Adjust your Social Security claim age to create a golden window for tax-free conversions
                              </p>
                            </div>
                          )}
                          
                          {/* NEW: Button for users who need to complete retirement optimization */}
                          {!hasValidOptimizationVars && anyoneMeetsAgeRequirement && (
                            <div className="mt-4">
                              <Button
                                onClick={() => window.location.href = '/retirement-planning?tab=optimization'}
                              ...
                              >
                                <Target className="h-4 w-4 mr-2" />
                                Complete Retirement Optimization
                              </Button>
                              <p className="text-xs text-gray-400 mt-2 text-center">
                                Complete and lock your retirement optimization variables to unlock Roth conversion analysis
                              </p>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="border border-gray-700 rounded-lg">
                        <Button
                          ...
                          variant="ghost"
                          className="w-full justify-between p-4 hover:bg-gray-800/50"
                        >
                          <div className="flex items-center">
                            <Lightbulb className="h-5 w-5 mr-2 text-blue-400" />
                            <span className="text-white font-medium">View Detailed Test Results</span>
                          </div>
                          {showTestDetails ? 
                            <ChevronUp className="h-5 w-5 text-gray-400" /> : 
                            <ChevronDown className="h-5 w-5 text-gray-400" />
                        ...
                        {showTestDetails && (
                          <div className="border-t border-gray-700 p-6 space-y-6">
                            {/* Test 1: Golden Window Test */}
                            <div className="space-y-3">
                              <h3 className="text-lg font-semibold text-white flex items-center">
                                <span className="bg-blue-500/20 text-blue-300 rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-3">1</span>
                                Golden Window Test
                              </h3>
                              <p className="text-sm text-gray-400 ml-11">
                                Golden Window represents the optimal period for Roth conversions - the time between retirement and Social Security when your income may be lower, allowing conversions at favorable tax rates.
                              </p>
                              
                              {/* User Golden Window */}
                              <div className="flex items-center justify-between p-4 bg-gray-800/30 rounded-lg border-l-4 border-l-blue-400">
                                <div>
                                  <h4 className="text-white font-semibold">Your Golden Window</h4>
                                  <p className="text-sm text-gray-400">
                                    Retirement at {retirementAge || 'Not set'} → Social Security at {socialSecurityClaimAge}
                                  </p>
                                </div>
                                <div className="text-center">
                                  <div className={`text-xl font-bold ${userPassesGapTest ? 'text-green-400' : 'text-orange-400'}`}>
                                    {userGapYears > 0 ? `${userGapYears} Years` : 'No Gap'}
                                  </div>
                                  <div className={`text-2xl ${userPassesGapTest ? 'text-green-400' : 'text-red-400'}`}>
                                    {userPassesGapTest ? '✅' : '❌'}
                                  </div>
                                </div>
                              </div>

                              {/* Spouse Golden Window (if married) */}
                              {userProfile?.maritalStatus === 'married' && userProfile?.spouseName && (
                                <div className="flex items-center justify-between p-4 bg-gray-800/30 rounded-lg border-l-4 border-l-blue-400">
                                  <div>
                                    <h4 className="text-white font-semibold">{userProfile.spouseName}'s Golden Window</h4>
                                    <p className="text-sm text-gray-400">
                                      Retirement at {spouseRetirementAge || 'Not set'} → Social Security at {spouseSocialSecurityClaimAge}
                                    </p>
                                  </div>
                                  <div className="text-center">
                                    <div className={`text-xl font-bold ${spousePassesGapTest ? 'text-green-400' : 'text-orange-400'}`}>
                                      {spouseGapYears > 0 ? `${spouseGapYears} Years` : 'No Gap'}
                                    </div>
                                    <div className={`text-2xl ${spousePassesGapTest ? 'text-green-400' : 'text-red-400'}`}>
                                      {spousePassesGapTest ? '✅' : '❌'}
                                    </div>
                                  </div>
                                </div>
                            ...
                            {/* Test 2: 3-Year Planning Window Test */}
                            <div className="space-y-3">
                              <h3 className="text-lg font-semibold text-white flex items-center">
                                <span className="bg-blue-500/20 text-blue-300 rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold mr-3">2</span>
                                Age 50+ Qualification Test
                              </h3>
                              <p className="text-sm text-gray-400 ml-11">
                                Roth conversion planning can begin at age 50+ when there's sufficient time to benefit from tax-free growth. You can start planning now and work with your advisor regardless of how many years remain until retirement.
                              </p>
                              
                              {/* User Age Test */}
                              <div className="flex items-center justify-between p-4 bg-gray-800/30 rounded-lg border-l-4 border-l-blue-400">
                                <div>
                                  <h4 className="text-white font-semibold">Your Age Qualification</h4>
                                  <div className="text-sm text-gray-400 space-y-1">
                                    <p>Current age: {currentUserAge} | Required: 50+</p>
                                    <p>Status: {userMeetsAgeRequirement ? '✓ Qualified for Roth conversion planning' : '✗ Under age 50'}</p>
                                  </div>
                                </div>
                                <div className="text-center">
                                  <div className={`text-xl font-bold ${userMeetsAgeRequirement ? 'text-green-400' : 'text-orange-400'}`}>
                                    {currentUserAge} Years Old
                                  </div>
                                  <div className={`text-2xl ${userMeetsAgeRequirement ? 'text-green-400' : 'text-red-400'}`}>
                                    {userMeetsAgeRequirement ? '✅' : '❌'}
                                  </div>
                                </div>
                              </div>

                              {/* Spouse Age 50+ Test (if married) */}
                              {userProfile?.maritalStatus === 'married' && userProfile?.spouseName && (
                                <div className="flex items-center justify-between p-4 bg-gray-800/30 rounded-lg border-l-4 border-l-purple-400">
                                  <div>
                                    <h4 className="text-white font-semibold">{userProfile.spouseName}'s Age Qualification</h4>
                                    <div className="text-sm text-gray-400 space-y-1">
                                      <p>Current age: {currentSpouseAge} | Retirement at: {spouseRetirementAge || 'Not set'}</p>
                                      <p>Tax-deferred assets: {spouseHasTaxDeferredAssets ? `✓ ${formatCurrency(spouseTaxDeferredAssets)}` : '✗ None found'}</p>
                                    </div>
                                  </div>
                                  <div className="text-center">
                                    <div className={`text-xl font-bold ${spouseMeetsAgeRequirement ? 'text-green-400' : 'text-orange-400'}`}>
                                      Age {currentSpouseAge}
                                    </div>
                                    <div className={`text-2xl ${spouseMeetsAgeRequirement ? 'text-green-400' : 'text-red-400'}`}>
                                      {spouseMeetsAgeRequirement ? '✅' : '❌'}
                                    </div>
                                  </div>
                                </div>
                              )}
                            ...
                            {(!retirementAge || !socialSecurityClaimAge || !currentUserAge) && (
                              <Alert className="bg-blue-500/10 border-blue-500/20">
                                <Info className="h-4 w-4 text-blue-300" />
                                <AlertDescription className="text-gray-300">
                                  <strong>Complete Your Profile:</strong> To get accurate assessment results, please complete your intake form including date of birth, desired retirement age, and Social Security claiming age.
                                </AlertDescription>
                              </Alert>
                            )}
                          </div>
