...
async function debugProjection() {
  try {
    ...
    const age66 = proj.find(p => p.age === 66);
    ...
    console.log("Age 66:", age66 ? `Savings: ${age66.savings}, NW: ${age66.totalNetWorth}` : "Not found");
    ...
    if (depleted) {
      ...
      // Show a few years before and after depletion
      const depletionIndex = proj.findIndex(p => p === depleted);
      for (let i = Math.max(0, depletionIndex - 2); i <= Math.min(proj.length - 1, depletionIndex + 2); i++) {
        const p = proj[i];
        console.log(`  Age ${p.age}: Savings=${p.savings}, RE=${p.realEstate}, Debt=${p.debt}, NW=${p.totalNetWorth}`);
      }
    }
